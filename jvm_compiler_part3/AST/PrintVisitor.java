...
public class PrintVisitor implements Visitor {	
	...
	public void visit(FormalParameterList fpl) {
		...
		if (fpl.noParams) {
			...
			for (int i=0;i<numParams;i++)
			{
				...
				FormalParameter fp = fpl.getAt(i);
				id = fp.id;
				Type type = fp.type;
				if (type.toString().equals("SimpleType")){
					SimpleType st = (SimpleType)fp.type;
					System.out.print(st.getType().toString());
				} else if ( type.toString().equals("ArrayType")){
	...
	public void visit(VariableDeclarationList varDeclList) {
		...
		for (int i = 0; i < sizeOfList; i++) {
			VariableDeclaration varDecl = varDeclList.getAt(i);
			varDecl.accept(this); 
		}
	...
	public void visit(VariableDeclaration vd) {
		printIndent();
		if (vd.type.toString().equals("SimpleType")) {
			SimpleType st = (SimpleType)vd.type;
			System.out.print(st.getType().toString());
		} else if ( vd.type.toString().equals("ArrayType")) {
